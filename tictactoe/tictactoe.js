  ...
  if (!playerTurn) {
    ...
    for (let letter of [O_TEXT, X_TEXT]) {
      for (let m of possibleMoves) {
        boardCopy = [];
        for (let i = 0; i < board.length; i++) boardCopy[i] = board[i];
    ...
    for (let i of possibleMoves) {
      if ([1, 3, 7, 9].includes(i)) cornersOpen.push(i);
    }
    ...
    let edgesOpen = [];
    for (let i of possibleMoves) {
      if ([2, 4, 6, 8].includes(i)) edgesOpen.push(i);
    }

    if (edgesOpen.length > 0) move = getRandomElement(edgesOpen);
